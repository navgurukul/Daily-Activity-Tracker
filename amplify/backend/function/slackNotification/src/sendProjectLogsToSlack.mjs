...
// const formatMessage = (projectName, logs, projectManagerEmail) => {
//   const managerSlackId = resolveSlackId(projectManagerEmail);
//   const managerTag = managerSlackId ? `<@${managerSlackId}>` : projectManagerEmail;

//   let message = `${managerTag}\n`;
//   message += `- here are the updates for your team for previous day. Please go through them properly. Unless you're adding anything here, this is your approval for this timesheet.\n`;
//   message += `Here's the update for previous day on *${projectName}*:\n`;
...
//   for (const [identifier, userLogs] of Object.entries(logsByUser)) {
//     const slackId = resolveSlackId(identifier);
//     const userTag = slackId ? `<@${slackId}>` : identifier;
...
//     message += `${userTag}\n`;
//     message += ` spent ${totalHours} hour${totalHours !== 1 ? 's' : ''} on the *${projectName}* on the previous day.\n`;
//     message += `   - Tasks:`;

//     // Compile all tasks from all logs for the user
//     let taskPoints = [];
//     userLogs.forEach((log) => {
//       let desc = log.workDescription || 'No description';
//       desc = desc.replace(/\r\n|\r|\n/g, '\n').trim();
